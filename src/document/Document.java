...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i = 0; i < wordChars.length; i++) {
			if(wordChars.length-1 == i && wordChars[i] == 'e' 
					&& isNewSyllable && syllablesCount > 0) {
				syllablesCount--;
			}
			if(isNewSyllable 
					&& vowels.indexOf(Character.toLowerCase(wordChars[i])) >= 0) {
				isNewSyllable = false;
				syllablesCount++;
			}
			else if(vowels.indexOf(Character.toLowerCase(wordChars[i])) < 0) {
				isNewSyllable = true;
			}
		}
