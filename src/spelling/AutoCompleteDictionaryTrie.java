...
public class AutoCompleteDictionaryTrie implements  Dictionary, AutoComplete {
	...
	{
	    char[] lowercase = word.toLowerCase().toCharArray();
	    TrieNode node = root;
	    
	    for(char letter: lowercase) {
	    	if(node.getValidNextCharacters().contains(letter)) {
     ...
     */@Override
     public List<String> predictCompletions(String prefix, int numCompletions) 
     {
    	...
    	char[] lowercase = prefix.toLowerCase().toCharArray();
 	    TrieNode node = root;
 	    ...
 	    for(int i = 0; i < children.size(); i++) {
 	    	char letter = children.get(i);
 	    	nodeQueue.add(node.getChild(letter));
 	    }
 	    ...
 	    while(!nodeQueue.isEmpty() && completions.size() < numCompletions) {
 	    	...
 	    	for(int i = 0; i < childNodes.size(); i++) {
 	    		char letter = childNodes.get(i);
 	    		nodeQueue.add(firstNode.getChild(letter));
 	    	}
 	    }
 	    return completions;
     }
